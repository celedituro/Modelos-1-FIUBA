...
s.t. AcumulacionIzq{i in BANCOS, j in BANCOS: j<>i and i<>'O' and j<>'O'}: -M * (1 - Y[i,j]) + DINERO[i] <= D[j] - D[i];
s.t. AcumulacionDer{i in BANCOS, j in BANCOS: j<>i and i<>'O' and j<>'O'}: D[j] - D[i] <= DINERO[i] + M * (1 - Y[i,j]);
